...
public class ParkingSpotRepository {
    ...
    public ParkingSpot getFreeParkingSpot(Long parkingPlaceId, VehicleType vehicleType) {
        ParkingSpotEntity freeSpot = parkingSpotDao.findFirstByParkingPlaceIdAndVehicleTypeAndIsFree(parkingPlaceId, vehicleType, true)
                .orElseThrow(() -> new RuntimeException("No Free spot available for vehicleType=" + vehicleType));
    ...
    public void initializeParkingSpotsForParkingPlace(ParkingPlace parkingPlace) {
        ...
        initialize(parkingPlaceId, parkingPlaceType, VehicleType.TWO_WHEELER, parkingPlace.getTwoWheelerCapacity());
        initialize(parkingPlaceId, parkingPlaceType, VehicleType.CARS, parkingPlace.getCarOrSuvCapacity());
        initialize(parkingPlaceId, parkingPlaceType, VehicleType.BUS, parkingPlace.getBusOrTruckCapacity());

    }

    private void initialize(Long placeId, ParkingPlaceType placeType, VehicleType vehicleType, Long capacity) {
    ...
    public void reserveParkingSpot(Long parkingSpotId) {
        ParkingSpotEntity spotEntity = parkingSpotDao.findById(parkingSpotId).orElseThrow(() -> new RuntimeException("Invalid stop id"));
        parkingSpotDao.save(spotEntity.toBuilder().isFree(false).build());
    }


    public void freeParkingSpotByName(String parkingSpotName) {
        ParkingSpotEntity spotEntity = parkingSpotDao.findByParkingSpotName(parkingSpotName).orElseThrow(() -> new RuntimeException("Invalid stop id"));
